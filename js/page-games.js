...
(async () => {
  ...
  JOIN_HTML = await fetch(
    chrome.runtime.getURL("resources/html/joining.html")
  ).then((x) => x.text());
  JOIN_JS = await import(chrome.runtime.getURL("resources/js/join.js"));
  PLAYER_IN_OTHER_GAME_HTML = await fetch(
    chrome.runtime.getURL("resources/html/player-in-other-game.html")
  ).then((x) => x.text());
  PLAYER_IN_OTHER_GAME_JS = await import(
    chrome.runtime.getURL("resources/js/player-in-other-game.js")
  );

  const winLocHash = window.location.hash?.split("&");
  if (winLocHash[1] && winLocHash[1].startsWith("rpss_join")) {
    // check direct join
  ...
  function createLayout() {
    ...
    function appendLayout() {
      fetch(chrome.runtime.getURL("resources/html/layout.html"))
        .then((result) => result.text())
        .then((RPSSlayout) => {
          const layout = document.createElement("div");
          const serverListOptions = document.getElementsByClassName(
            "server-list-options"
          )[0];
