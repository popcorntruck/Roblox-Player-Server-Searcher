  ...
  await chrome.storage.local.get(null, async (RPSS) => {
    ...
    switch (result[0]) {
      ...
      default:
        ...
        const username =
          result[0].toString().length > 1 ? result[2] : result[3];

        JOIN_JS.join(gameId, avatar, username);
        break;
    }
    if (typeof result[0] === "number" && result[0] <= 5)
      warningContainer.style.transform = "translateX(0px)";
...
function showEULA(RPSS) {
  fetch(chrome.runtime.getURL("resources/html/eula.html"))
    ...
    .then((html) => {
      ...
      body.firstChild
        ? body.insertBefore(eulaAlert, body.firstChild)
        : body.appendChild(eulaAlert);
      eulaAlert.children[0].style.height = `${
        eulaAlert.children[0].offsetHeight * 0.8
      }px`;
      ...
      agreeCheckbox.addEventListener("change", (event) => {
        ...
        countdown.style.visibility =
          countdown.style.visibility === "visible" ? "" : "visible";
...
async function findPlayer() {
  ...
  const avatarImageUrl =
    playerAvatarHeadshot.state === "Completed"
      ? playerAvatarHeadshot.imageUrl
      : chrome.runtime.getURL("resources/svg/error-404.svg");
  ...
  async function fetchPlayerAvatarHeadshot() {
    ...
    if (!username)
      username = await fetch(
        `https://users.roblox.com/v1/users/${targetPlayer}`
      )
        .then((x) => x.json())
        .then((res) => res.name);
    ...
    return await fetch(
      ...
      .then((res) => {
        if (res.errors || res.data.length === 0)
          return (targetPlayer = undefined); // if userId doesn't exist
        return res.data[0];
      });
  }
  ...
  async function checkStatus() {
    return new Promise((resolve, reject) => {
      chrome.runtime.sendMessage(
        { msg: "Fetch_PresenceApi", content: targetPlayer },
        (response) => {
  ...
  async function fetchServers(nextPageCursor) {
    await fetch(
      `https://games.roblox.com/v1/games/${PLACE_ID}/servers/0?limit=100&cursor=${nextPageCursor}`
    )
      .then((x) => x.json())
      .then(async (servers) => {
  ...
  async function fetchTokens() {
    ...
    async function checkUserBatch(slice, attempts = 1) {
      ...
      try {
        ...
        const targetUserData = res.data.find(
          (data) => data.imageUrl === avatarImageUrl
        );
    ...
    async function processSlices(slices) {
      const results = await Promise.all(
        slices.map((slice) => checkUserBatch(slice))
      );
    ...
    for (
      ...
      for (
        let requestIndex = 0;
        requestIndex < maxParallelRequests;
        requestIndex++
      ) {
        const start = currentIndex + requestIndex * 100;
        if (start >= allTokens.length) break;
        slices.push(
          allTokens.slice(start, Math.min(start + 100, allTokens.length))
        );
      }
