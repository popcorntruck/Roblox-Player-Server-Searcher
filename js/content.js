...
function autoCheckUpdate() {
  ...
  chrome.storage.local.get(null, async (RPSS) => {
    ...
    const availableVersion = await fetch(
      `https://api.github.com/repos/${githubOwner}/${githubRepo}/releases/latest`
    ).then(async (response) => {
      const isRateLimitExceeded = response.status === 403;
      const rateLimitReset =
        Number(response.headers.get("x-ratelimit-reset")) - 1740;
    ...
    if (currentVersion !== availableVersion) {
      ...
      container.children[0].addEventListener("click", async () => {
        // close alert
  ...
  async function showUpdate(currentVersion, availableVersion) {
    return await fetch(chrome.runtime.getURL("resources/html/new-update.html"))
      .then((x) => x.text())
      .then((html) => {
        const updateAlert = document.createElement("div");
        const body = document.body;
        body.firstChild
          ? body.insertBefore(updateAlert, body.firstChild)
          : body.appendChild(updateAlert);
        updateAlert.outerHTML = html;

        const container = document.getElementById("RPSS-ACU-Container");
        container.children[0].style.backgroundImage = `url(${chrome.runtime.getURL(
